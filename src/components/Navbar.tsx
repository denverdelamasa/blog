...
export default function Navbar() {
  ...
  return (
    <nav className="flex flex-col m-2 rounded-[4px] border-4 border-[#202020] bg-orange-400 min-h-[64px] transition-all duration-500 ease-in-out overflow-hidden">
      <div className="flex flex-row items-center justify-between w-full min-h-[64px]">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 m-4">
          ...
          <button
            className="md:hidden w-[42px] h-[42px] m-2 neo-brutalist hover:bg-orange-500 bg-orange-400 flex items-center justify-center transition-all duration-300"
            onClick={toggleNavbar}
            aria-label="Toggle menu"
            aria-expanded={isExpanded}
            aria-controls="mobile-menu"
          >
            <i className={`bi ${isExpanded ? 'bi-x' : 'bi-list'} text-black text-2xl transition-transform duration-500 ${isExpanded ? 'rotate-180' : ''}`}></i>
          </button>
        </div>
      </div>
